...
{ private Produkt[] base;   // the collection of records
  ...
  { if ( initial_size > 0 )
         { base = new Produkt[initial_size]; }
    else { base = new Produkt[1]; }
  }
  ...
  private int findLocation(Barkodi k)
  { int result = NOT_FOUND;
    boolean found = false;
    int i = 0;
    while ( !found  &&  i != base.length )
          { if ( base[i] != null  &&  base[i].getBarcode().equals(k) )
                 { found = true;
                   result = i;
                 }
            else { i = i + 1; }
 ...
 public boolean insert(Produkt r)
 { boolean success = false;
   if ( findLocation(r.getBarcode()) == NOT_FOUND )  // r  not already in  base?
      { // find an empty element in  base  for insertion of  r:
        ...
        if ( found_empty_place )
             { base[i] = r; }
               ...
               Produkt[] temp = new Produkt[base.length * 2];
 ...
 public boolean delete(Barkodi k)
